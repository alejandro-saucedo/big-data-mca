...
BASE_PATH = '/work/data/AGOSTO_2022'

WRITE_PATH = '/work/data/AGOSTO_2022_PARQUET/'
...
folders = [os.path.join(BASE_PATH, f) for idx, f in enumerate(os.listdir(BASE_PATH)) if idx < 10]
print(folders)
...
for folder, files in json_files.items():
    ...
    .selectExpr(
    	'`ID EQP` as EQP_ID',
    	'`DATA HORA` as DATE_TIME',
    	'MILESEGUNDO as MILLISECOND',
    	'`CLASSIFICAÇÃO` as CLASSIFICATION',
    	'FAIXA as ROAD_LANE',
    	'`ID DE ENDEREÇO` as ADDRESS_ID',
    	'`VELOCIDADE DA VIA` as ROAD_SPEED',
    	'`VELOCIDADE AFERIDA` as VEHICLE_SPEED',
    	'TAMANHO as VEHICLE_LENGTH',
    	'`NUMERO DE SÉRIE` as SERIAL_NUMBER',
    	'LATITUDE',
    	'LONGITUDE',
    	'`ENDEREÇO` as ADDRESS',
    	'SENTIDO as DIRECTION')\
    .write\
    .mode('overwrite')\
